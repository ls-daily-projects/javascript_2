...
let allCaps = runners.map(runner => {
    const newRunner = Object.assign(runner)
    newRunner.first_name = runner.first_name.toUpperCase()
    return newRunner
})
console.log(allCaps)
...
let ticketPriceTotal = runners.reduce(
    (acc, current) => acc + current.donation,
    0
)
...
// Problem 2
const domains = emailList.map(email => email.split("@")[1])
console.log(domains)

// Problem 3
const uniqueDomains = Array.from(
    domains.reduce((acc, current) => acc.add(current), new Set())
)
console.log(
    uniqueDomains,
    `${runners.length} domains total => ${
